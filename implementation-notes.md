...
- [결정] 2026-07-16 | **재시작 파이프라인 갭 감사 → 4건 봉합** (유저 요청 "빠진 것 검토") | 실물 대조 발견: ①2차 -9에 micro_ros_agent·ardupilot_sitl·relay·rmw_zenohd 누락(1차만 있음 — XRCE 잔존 시 세션충돌·SHM 재누적 위험)→추가(10→14종) ②검증 리포트 패턴에 relay·rmw_zenohd 누락→추가 ③재시작 절차에 eeprom 함정 크로스링크 없음("재시작≠parm 반영")→RULES에 추가 ④start_sim 프리플라이트 fail-open(xml 부재 시 무검사 통과)→fail-fast 추가. ⑤armed(비행 중) 자동 가드는 과잉 엔지니어링 위험으로 규율 유지(미적용). 인터페이스 불변(내부 경화만), bash -n 통과. 다음 stop부터 적용 | (감사 2026-07-16)
- [결정] 2026-07-16 | **재시작 arducopter 교체 검증 규율 확립 + 전 문서 반영** | 재시작마다 `PID 전→후 · 나이(etime)` 표기 — 새 PID+초 단위=fresh 정상, 분 단위=미교체 신호. 실증 사슬 4회: 43548→52540→57745→64512(각 나이 10~45s, stop 후 매번 arducopter 0 검증). 유저 의심("재시작 시 ardupilot도 초기화되나")에 대한 실측 답: **정상 교체됨** — 34분 나이는 버그 아닌 실제 경과시간. 반영: CLAUDE §3·RULES 재시작절차(5단계로 확장)·known-good §7(⑥ 신설)·session-restart-protocol(5단계) | (파일 메모리 feedback_restart_arducopter_verify)
- [테스트] 2026-07-16 | **집 fresh RTF ~1.0 실측** + 34분 구동 "송신 적어짐"=누적저하 재확인 | 집(en0 45.50↔45.93, rate10): fresh 직후 순간 RTF 0.999~1.0003, gz 렌더 9.4Hz(rate10 목표 정상), compressed 송신 확인(jpeg bgr8). 34분 구동 시 "송신 적어짐" 체감=기록된 누적저하 → clean 재시작으로 리셋. **RTF는 환경 아닌 호스트 부하 의존**(집 fresh 1.0 vs 사무실 0.50). 성능 정본 §2b에 RTF 열 추가 | (실측 2026-07-16)
- [결정] 2026-07-16 | 모델 Fable 5 전환 + 세션 핸드오프 블록 도입 | 전환 전 project_ardu_ws.md 최상단에 "▶ 세션 핸드오프" 블록(현 위치·성과·미해결·규율) 작성 → 전환 후 4항목 실물 재검증 전부 일치(핸드오프 방식 유효 실증). RULES CycloneDDS 예시의 en7 하드코딩 → "자동: 내 IP 소유 NIC"로 정정 | (2026-07-16)
- [테스트] 2026-07-15 | **카메라 update_rate가 RTF 지배 요인 확정(정정)** — 30 원복=10 | 깨끗한 15초 실효 RTF A/B: **rate 10 → RTF 0.47~0.50, rate 30 → RTF 0.20** (2.5배 차). 주말 update_rate 10→30 변경이 RTF 반토막→드론 벽시계 이동 느림의 주범. **10 원복 확정**(양 트리). ⚠️ 2026-07-13 "update_rate 30 유지·bridge가 상한"·본 세션 초반 "원복해도 RTF 무변화"는 **둘 다 오판**(지터 단발 측정) — RTF는 카메라 렌더 부하에 강하게 반응. 남은 레버: 해상도↓(640×480→320×240=렌더 1/4)면 RTF 추가 상승. 배터리·발열·저전력은 무죄(AC·미스로틀 실측). 물리설정(1ms/1000Hz/lock_step1)·압축(2.3Mbps)은 무관 | (clean A/B 실측 2026-07-15)
- [테스트] 2026-07-15 | **사무실/랩 유선 전환 — 저쪽 처리율 130~150fps** + 멀티호밍 disambiguation 실증 | 내 **en7 10.130.200.31** ↔ 저쪽 **10.130.200.36**(유선, RTT 0.8ms). 랩은 en7(.31)·en0(.30) **동시 같은 서브넷** → `sync_and_build 10.130.200.36 10.130.200.31`가 내 IP(.31)로 **en7 정확 특정**(en0/.30 오선택 방지) — 방법론 2번째 인자 실전 성공. 부팅 DDS/EKF3/republisher·바인딩에러 0, compressed 발행 확인, gz 14.6Hz(RTF 0.46), arducopter 1(pgrep 2는 바이너리+래퍼, 포트 5760 단독=단일 확정). 저쪽 처리율 **130~150fps**(⚠️ 전달률 아님 — 처리/검출 루프, 유선 저지연이라 무선 60~80보다 빠름). 환경별 성능표: `Docs/setups/2026-07-14-known-good-crossmachine-wifi-sample.md` §2b | (실측 2026-07-15)
