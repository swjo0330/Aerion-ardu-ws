...
## 🔒 고정 기준값 (현 환경 baseline — 함부로 바꾸지 말 것)

> 2026-07-15 **사무실/랩 유선** 실측 고정값. 아래는 known-good — 변경 시 성능·연결 회귀. 바꿔야 하면 이 표를 근거로 A/B 후에만 갱신.
> 환경: 내 **en7 10.130.200.31** ↔ 저쪽 **10.130.200.36** (멀티호밍 — 내 IP 지정으로 en7 특정)

**고정 설정 (변경 금지):**

| 항목 | 값 | 근거 |
|---|---|---|
| 카메라 `<update_rate>` | **10** | 30이면 RTF 0.20으로 반토막(clean A/B 2026-07-15). `gimbal_small_3d/model.sdf` 양 트리 |
| 카메라 해상도 | **640×480** | 올리면 렌더부하↑ → RTF↓ |
| cyclonedds `MaxMessageSize`/`FragmentSize` | **1400B / 1344B** | IP fragmentation 회피 |
| DDS Peer | **유니캐스트** (디폴트 멀티캐스트 금지) | 공유망 flooding 방지 |
| DDS NetworkInterface | **en7** (내 IP 소유 NIC, sync_and_build 자동) | 멀티호밍서 en0 오선택 방지 |
| MAVProxy out | **10.130.200.36:14555** | |
| lock_step (단일모드) | **1** | 멀티는 0 |

**실측 baseline (현 환경, rate 10 기준):**

| 지표 | 값 |
|---|---|
| 실효 RTF | **~0.50** |
| gz 카메라 렌더 | ~10Hz(sim-time) / ~5Hz(wall @ RTF 0.5) |
| 저쪽 처리율 FPS | **130~150** (유선 저지연) |
| 카메라 전달률 | ≈ 렌더율(≤10Hz) |
| 저쪽 RTT | 0.8ms |

> ⚠️ RTF ~0.50은 **이 sim의 현 노트북 정상치**(집에서도 0.56~0.93 변동). AC·미스로틀 확인됨. 더 올리려면 카메라 해상도↓가 유일 레버.
...
- **⚠️ RTF(드론 체감속도) = 카메라 `<update_rate>`가 지배** (2026-07-15 clean A/B): **rate 10 → RTF 0.47~0.50, rate 30 → RTF 0.20**(2.5배). RTF는 시간 배율이라 낮으면 드론이 벽시계 기준 느리게 재생됨(제어는 정상). **update_rate는 10 유지**(주말 30이 RTF 반토막 주범, 원복 확정). 배터리·발열·압축·물리설정은 무죄. 더 올리려면 해상도 640×480→320×240. 상세: [../RULES.md](../RULES.md) "드론이 느리게 움직임" 항목.
...
| 환경 | 내 IP / NIC | 저쪽 IP | 매체 | RTT | RTF(rate10·fresh) | 저쪽 **처리율 FPS** | 카메라 전달률(≈소스) |
|---|---|---|---|---|---|---|---|
| **집 (Wi-Fi)** | 192.168.45.50 / en0 | 192.168.45.93 | 무선 양단 | ~10-13ms | **~1.0** (2026-07-16 실측) | **60~80** | ≈ gz 렌더 (rate10 fresh ~9.4Hz) |
| **엄마집 (Wi-Fi)** | 192.168.35.152 / en0 | 192.168.35.208 | 무선 양단 | **5.5ms** | (미실측) | (미실측) | — |
| **사무실/랩 (유선)** | 10.130.200.31 / **en7** | 10.130.200.36 | 유선 | **0.8ms** | **~0.50** | **130~150** | ≈ gz 렌더 ~14~22Hz |

- 장소 3곳 확정: **랩 10.130.200.x(en7 유선·멀티호밍 주의) / 집 192.168.45.x(en0) / 엄마집 192.168.35.x(en0)** — 어느 곳이든 시작 IP 루프(`sync_and_build <저쪽IP> <내IP>`)가 동일하게 자가치유(2026-07-16 엄마집 첫 적용 성공). 과거 2026-06-27 "집 35.158" 기록의 35.x 대역 = 엄마집 망.

- RTF는 환경 아닌 **호스트 부하** 의존 — 집 fresh에서 1.0, 사무실(부하 高)에서 0.50 실측. rate 30이면 어디서든 반토막(§고정 기준값).

- ⚠️ **처리율 FPS ≠ 카메라 전달률**: 60~80·130~150은 저쪽 **처리/검출 루프 처리량**(입력 고속 재처리)이지 카메라 프레임 전달률이 아님. 전달률 상한 = gz 소스 렌더율(~14~22Hz, RTF 의존). 유선(0.8ms)이 무선보다 저지연이라 저쪽 루프가 더 빨리 돎(60~80→130~150).
- **사무실 멀티호밍 주의**: 랩에선 en7(10.130.200.31)·en0(10.130.200.30)이 **동시에 같은 서브넷** → 반드시 `sync_and_build <저쪽IP> <내IP>`로 내 IP(.31) 지정해 **en7 특정**(en0/.30 오선택 방지). 방법론 실증됨(2026-07-15).
...
**⑥ arducopter 교체 검증 (기동 후, 2026-07-16 규율):** `ps -o pid,etime -p $(pgrep -f arducopter|head -1)` → **새 PID + 초 단위 나이 = fresh 정상**, 분 단위 = 미교체 신호. 매 재시작 보고에 `PID 전→후 · 나이` 표기. SITL 상태(eeprom·카메라 누적저하)가 새 프로세스로 리셋됐는지의 최종 증거 (실증 사슬: 43548→52540→57745→64512).

주의: 재시작은 **한 번에 하나씩**(`&` 남발 시 SITL 중복 누적) · **저쪽 비행 중 재시작 금지**(FCU 부팅=미션 소실) · **장시간(30분+) 구동 시 카메라 발행 Hz 누적 저하** → 실험·측정 전 clean 재시작이 정본.
