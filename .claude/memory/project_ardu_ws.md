...
**▶ 세션 핸드오프 (2026-07-20 갱신 — 새 세션은 이 블록부터):**
- **현 위치**: **회사/랩** (장소 4안: 랩 10.130.200.x·en7유선 / 집 45.x·en0 / 엄마집 192.168.35.x·en0 — 어디든 시작 IP 루프가 자가치유). 내 **en7 10.130.200.27** ↔ 저쪽 **10.130.200.28** (유선, RTT 0.8ms). ⚠️**멀티호밍**: en0(.26)+en7(.27) 동시 10.130.200.x → 내 IP(.27)로 en7 특정. sim **가동 중** — arducopter **PID 80464**(fresh: 58422[2일9h]→0→80464·10s). update_rate **10** 고정.
- **모델**: Opus 4.8↔Fable 5 왕복 실증(하네스는 모델 불문, 파일 기반). 2026-07-16 Fable 전환→2026-07-17 Opus 복귀, 핸드오프 무손실.
- **신규 규율(2026-07-16)**: 재시작마다 **arducopter PID 전→후·나이 표기**(초=fresh/분=미교체 신호) — CLAUDE §3·RULES 재시작절차·known-good §7·restart-protocol 전 반영([[feedback_restart_arducopter_verify]]). 장시간(30분+) 구동=카메라 발행 누적저하 → 측정 전 clean 재시작.
- **이번 세션 성과 (커밋 `e02dfd7` push 완료)**: ①시작 IP 방법론+자원프리 루프 **하네스화**([[feedback_startup_ip_methodology]]) ②회사망 포화 근본원인=raw 80Mbps wire 유출([[feedback_dds_raw_lan_flood]]) ③**RTF↔카메라 update_rate 확정**(clean A/B: 10→RTF0.5, 30→0.2 — 10 고정, 드론 느림의 주범) ④known-good 성능 정본 `Docs/setups/2026-07-14-known-good-crossmachine-wifi-sample.md`(🔒고정 기준값+환경별 성능표: 집Wi-Fi 처리율 60~80/사무실유선 130~150) ⑤재시작 arducopter PID·나이 검증([[feedback_restart_arducopter_verify]])
- **미해결/다음 착수점**: ①**사무실 미션 업로드 부분실패**(저쪽→SITL, DENIED/CANCELLED 실측) — 멀티호밍(en7+en0 동시 10.130.200.x) 유력, **en0 down 실험 대기**(사무실 복귀 시). 우리측 방화벽off·라우트en7 무결 확인함 ②fps/RTF 추가 개선 레버=**카메라 해상도 640×480→320×240** 또는 bridge MultiThreadedExecutor(미착수) ③update_rate 10 원복은 nested repo(src/install gitignore)라 미커밋 — 문서로만 박제(clean clone 시 gen 파생)
- **운영 규율(고정)**: 시작 시 유저가 **양 IP 통보** → `sync_and_build <저쪽IP> <내IP>`(내IP로 NIC 특정, 멀티호밍 대비); 재시작=`stop_sim` 경유+**arducopter PID·나이 표기**(초=fresh); **저쪽 비행 중 재시작 금지**(FCU 부팅=미션 소실); 공유망 Peer 유니캐스트 유지.

**이전 상태 (2026-07-15, 사무실/랩 유선):** 이 Mac **en7 10.130.200.31** ↔ 저쪽 **10.130.200.36**(유선, RTT 0.8ms). ⚠️ **멀티호밍**: en7(.31)·en0(.30) 동시 같은 서브넷 → `sync_and_build 10.130.200.36 10.130.200.31`로 내 IP(.31) 지정해 **en7 특정**(방법론 2번째 인자 실증). 저쪽 처리율 **130~150fps**(⚠️ 전달률 아님 — 처리/검출 루프, 유선 저지연이라 무선 60~80보다 빠름). 환경별 성능·IP표: `Docs/setups/2026-07-14-known-good-crossmachine-wifi-sample.md` §2b.
  - ⚡ **RTF↔카메라 update_rate 확정(2026-07-15 clean A/B, 정정)**: **rate 10→RTF 0.47~0.50, rate 30→RTF 0.20**(2.5배). RTF=시간배율→낮으면 드론 벽시계 느림(제어 정상). **update_rate 10 유지**(주말 30이 주범, 원복). 2026-07-13 "30 유지·bridge 상한" 노트 정정. 배터리·발열·압축·물리설정 무죄. 추가 레버=해상도↓. 상세: RULES "드론이 느리게 움직임".

**이전 상태 (2026-07-14, 집/Wi-Fi — known-good 샘플):** 이 Mac **en0 192.168.45.50 (Wi-Fi)** ↔ 저쪽 **192.168.45.93 (Wi-Fi, 양단 무선)**. `/camera/image/compressed`(JPEG) 저쪽 **카메라 전달률 18~25fps 검증**, 저쪽 **처리율 FPS 60~80 도달**(매우 양호, 사용자 확인). ⚠️ **60~80은 전달률 아님 — 소스 렌더 ~22Hz이므로 저쪽 처리/검출 루프 처리량**(입력 고속 재처리; 파이프라인이 저쪽 처리를 병목 없이 공급한다는 신호). **재현 샘플 정본: `Docs/setups/2026-07-14-known-good-crossmachine-wifi-sample.md`** (IP 포함 템플릿·연결단계·주의점).
  - ⚡ **시작 IP 방법론 도입(2026-07-14, [[feedback_startup_ip_methodology]])**: `sync_and_build <저쪽IP> <내IP>`가 **NetworkInterface를 내 IP 소유 NIC로 자동 재작성**(멀티호밍·가상NIC 배제) + start_sim 프리플라이트(죽은 NIC fail-fast). 하네스(CLAUDE §3/§4)·session-restart-protocol 반영. **→ 아래 옛 기록의 "sync는 NIC 안 건드림·수동 전환·NetworkInterface en7 하드코딩" 전부 무효.**
  - ⚡ **정정: "Wi-Fi DDS 금지" 무효** — **compressed는 Wi-Fi 크로스머신 18~25fps 정상**(양쪽 Wi-Fi 실증). raw-over-Wi-Fi도 "불가"로 단정 금지: 과거 불안정 주원인은 **설정**(Peer stale·부재/오NIC·멀티호밍·소비자AP)으로 규명, 매체 한계 아님([[dds-wifi]] 480p raw 성공 반례). compressed는 저대역 안전 기본값일 뿐.
  - ⚡ **정정: fps 병목 = 브리지 단일스레드 + 호스트부하** (하드 8~12Hz 상한 아님. load 낮으면 소스 22Hz 근접, load 19.7·동시 측정프로브면 4Hz로 급락 — 측정행위 자체가 fps 떨어뜨리는 함정). 토픽명 변경은 지연 무관(DDS 라벨).
  - Tailscale류 VPN(utun·100.x) 존재하나 방법론이 배제하므로 오선택 없음.
